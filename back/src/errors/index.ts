...
export const mimetype_not_allowed = (data?: IErrData) => {
  const err = new HttpException(
    {
      status: 400,
      error: data?.msg || 'MIMETYPE_NOT_ALLOWED',
    },
    400
  );
...
export const jwt_token_expired = (data?: IErrData) => {
  const err = new HttpException(
    {
      status: 403,
...
export const refresh_token_expired = (data?: IErrData) => {
  const err = new HttpException(
    {
      status: 419,
