...
import { account_blocked, authorization_failed, bad_request, refresh_token_expired } from '../../errors';
...
export class OAuthService {
  ...
  public async signInByRefreshToken(refresh_token: string) {
    return await this.dataSource.transaction(async (entityManager) => {
      ...
      if (!this.checkExpiresAt(current_refresh_token.expires_at)) {
        refresh_token_expired({ raise: true });
      }
  ...
  private async generetaJwt(entityManager: EntityManager, user: User) {
    const refresh_token = await entityManager.getRepository(RefreshToken).save({ user_id: user.id });
